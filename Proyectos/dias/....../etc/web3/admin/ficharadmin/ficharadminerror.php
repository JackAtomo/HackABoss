...
<select name="id_trabajador" id=obra1 required>
<?php // ponemos como opcion el resto de las obras 
$ssql = $mysqli -> query ("SELECT nombre , usuario , id_trabajador FROM trabajadores WHERE activo=1 ORDER BY nombre ASC ");
while ($dat = mysqli_fetch_array($ssql)){ 
echo '<option value="'.$dat['id_trabajador'].'">' .$dat['usuario']."---".$dat['nombre'].'</option>';	
}
?>
		</select>
					</p>
...
?> 	
					...
					<p>
						<label>Hora incio intermedio :</label>
<?php
